...
def getBenchmarkID(targetOS,osVersion):
    conn = pymssql.connect('DEV_CMDB_AG','cmdb','v13w26!','CMDB')
    cursor = conn.cursor()
    cursor.execute("SELECT BENCHMARK_UUID FROM SECURITY_POLICY WHERE OS = '" + targetOS + "' AND OS_VERSION = '" + osVersion + "'")
    row = cursor.fetchone()
    return row[0]
    conn.close

def getCheckID(targetOS,osVersion):
    conn = pymssql.connect('DEV_CMDB_AG','cmdb','v13w26!','CMDB')
    cursor = conn.cursor()
    cursor.execute("SELECT DISTINCT SPM.CHECK_UUID FROM SECURITY_POLICY_MAP SPM INNER JOIN SECURITY_POLICY SP ON SP.BENCHMARK_UUID=SPM.BENCHMARK_UUID AND SP.ENABLED='TRUE' INNER JOIN SECURITY_CHECK SC ON SC.CHECK_UUID= SPM.CHECK_UUID AND SC.EXEMPT='FALSE' WHERE SP.OS = '" + targetOS + "' AND SP.OS_VERSION = '" + osVersion + "'")
    chk = cursor.fetchall()
    checks = []
    for row in chk:
        checks.append(row[0])
