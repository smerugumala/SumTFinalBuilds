...
def reAssessPolicy(policy_name):
    ...
    client.api.sec.assess_policy(policy_uuid)
...
def deletePolicy(policy_name):
    ...
    for policy in policies:
        policy_uuid=policy["uuid"]
    client.api.sec.delete_policy(policy_uuid)
...
def deleteTarget(policy_name):
    ...
    client.api.tgt.delete_target_group(target_uuid)